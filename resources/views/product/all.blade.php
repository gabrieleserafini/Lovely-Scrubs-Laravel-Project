  ...
  <div class="container marketing pt-5">
    ...
    <div class="row">
        ...
        @foreach($products as $product)
            ...
            <div class="col-lg-4">
                <img class="bd-placeholder-img rounded-circle shadow mb-1 bg-body rounded" width="200" height="200" src="{{ $product->image }}" focusable="false">

                <h2><a href="/products/{{ $product->id }}" class="text-reset text-decoration-none">{{ $product->name }}</a></h2>
                <p>{{ $product->description }}</p>
                <p> <strong> € {{ $product->price }}</strong></p>
                <p><a class="btn btn-secondary" href="/products/{{ $product->id }}">View details &raquo;</a></p>
            </div><!-- /.col-lg-4 -->
    ...
    @foreach($products as $product)
        ...
        <div class="row featurette">
            <div class="col-md-7 order-md-2">
                <h2 class="featurette-heading">{{ $product->name }} <span class="text-muted">Made for you.</span></h2>
                <p class="lead">{{ $product->description }}</p>
                <p>
                  <span class="p-1"> <strong> € {{ $product->price }}  </strong>  </span> 
                ...
                <div class="pt-2">
                  <p><a class="btn btn-lg btn-secondary" href="/products/{{ $product->id }}">View details &raquo;</a></p>
                </div>
            </div>
            <div class="col-md-5 order-md-1">
                <img class="bd-placeholder-img bd-placeholder-img-lg featurette-image img-fluid mx-auto" width="500" height="500" src="{{ $product->image }}" focusable="false">
            </div>
        </div>
